...
public class ClienteApiController implements ClienteApi {
    ...
    private final ClienteApiDelegate delegate;

    public ClienteApiController(@org.springframework.beans.factory.annotation.Autowired(required = false) ClienteApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new ClienteApiDelegate() {});
    }

    @Override
    public ClienteApiDelegate getDelegate() {
        return delegate;
    }
